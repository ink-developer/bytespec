...
Для сборки документации нужен Python 3.11+ (требование sphinx-design 0.7).
Сама библиотека поддерживает Python 3.10+. Из корня репозитория выполните:
...
Элементы интерфейса в RST
-------------------------

Используйте ``tab-set`` с вкладками Python, Байты и Разбор только там,
где полезно сопоставить код с результатом. ``testcode`` и ``testoutput``
внутри вкладок по-прежнему исполняются doctest, даже если вкладка скрыта.
Независимым примерам задавайте одинаковое имя группы у кода и ожидаемого вывода.

Для разбора байтов используйте ``container:: wire-bytes`` с короткими
``dropdown`` внутри. Номер, имя поля и hex находятся в заголовке, объяснение —
в раскрываемой части. Для пояснений к коду используйте тот же приём с
``container:: code-notes``; номера остаются вне копируемого Python-кода.
Схема заголовка использует ``grid`` с классом ``wire-layout``.
Общие стили находятся в ``_static/interactive.css``; raw HTML не нужен.

Пример повторяемого блока:

.. code-block:: rst

   .. container:: wire-bytes

      .. dropdown:: ① value · ``00 00 00 2a``

         UInt32: 42, four bytes.

Текст интерфейса пишется в RST и попадает в gettext. После изменения
примеров проверяйте обе языковые версии, узкий экран, клавиатуру,
копирование, печать и чтение без JavaScript. Вкладки и раскрываемые блоки
работают средствами HTML/CSS; собственный JavaScript не требуется.
